...
# -----------------------
ZED_CSV = r"D:\Pycharm Projects\Hand_pose_estimation\new_project\main_char_new_dataset_creation\Test_dataset\yolo_8_m\auto_labelled_dataset\hand_keypoints_with_gestures.csv"
#HAND_CSV = "D:/Pycharm Projects/Hand_pose_estimation/new_project/main_char_new_dataset_creation/Test_dataset/labelled_dataset/hand_keypoints_gestures.csv"
IMAGES_FOLDER = "D:/Pycharm Projects/Hand_pose_estimation/new_project/main_char_new_dataset_creation/Test_dataset/labelled_dataset/images"
...
hand_cols = [
    "frame", "hand_gesture",
    "hand_kpt0_x", "hand_kpt0_y", "hand_kpt0_z",
    "hand_kpt1_x", "hand_kpt1_y", "hand_kpt1_z",
    "hand_kpt2_x", "hand_kpt2_y", "hand_kpt2_z",
    "hand_kpt3_x", "hand_kpt3_y", "hand_kpt3_z",
    "hand_kpt4_x", "hand_kpt4_y", "hand_kpt4_z",
    "hand_kpt5_x", "hand_kpt5_y", "hand_kpt5_z",
    "hand_kpt6_x", "hand_kpt6_y", "hand_kpt6_z",
    "hand_kpt7_x", "hand_kpt7_y", "hand_kpt7_z",
    "hand_kpt8_x", "hand_kpt8_y", "hand_kpt8_z",
    "hand_kpt9_x", "hand_kpt9_y", "hand_kpt9_z",
    "hand_kpt10_x", "hand_kpt10_y", "hand_kpt10_z",
    "hand_kpt11_x", "hand_kpt11_y", "hand_kpt11_z",
    "hand_kpt12_x", "hand_kpt12_y", "hand_kpt12_z",
    "hand_kpt13_x", "hand_kpt13_y", "hand_kpt13_z",
    "hand_kpt14_x", "hand_kpt14_y", "hand_kpt14_z",
    "hand_kpt15_x", "hand_kpt15_y", "hand_kpt15_z",
    "hand_kpt16_x", "hand_kpt16_y", "hand_kpt16_z",
    "hand_kpt17_x", "hand_kpt17_y", "hand_kpt17_z",
    "hand_kpt18_x", "hand_kpt18_y", "hand_kpt18_z",
    "hand_kpt19_x", "hand_kpt19_y", "hand_kpt19_z",
    "hand_kpt20_x", "hand_kpt20_y", "hand_kpt20_z"
]
df_hand = df_zed[hand_cols]

# build mapping from image->GT hand row (if multiple rows per image keep first; adapt if needed)
gt_hand_by_image = df_hand.set_index("frame").to_dict(orient="index")
...
images = sorted(list(set(df_hand["frame"].tolist())))  # iterate over images that have a GT hand
print(f"Evaluating {len(images)} images...")

for img_name in images:
    # get all humans in this frame
    humans = df_zed[df_zed["frame"] == img_name]
    if humans.empty:
        print('no humans detected in this image :', img_name)
    ...
    for idx, row in humans.iterrows():             # here it iterates over all the humans from the zed csv file
        ...
        print('pred is :', pred)
        main_gt_labels.append(row["zed_human_label"])
    ...
    if best_candidate is None:
        # no candidate, skip
        continue
    print('best candidate value is :', best_candidate)
    ...
    gt_bbox = [int(gt["bbox_x1"]), int(gt["bbox_y1"]), int(gt["bbox_x2"]), int(gt["bbox_y2"])]
    gt_gesture = gt["hand_gesture"]
...
for row in per_image_rows:
    ...
    if row["pred_bbox_x1"] is not None:
        ...
        humans = df_zed[df_zed["frame"] == row["frame"]]
        if not humans.empty:
    ...
    out_path = os.path.join(VIS_FOLDER, row["frame"])
