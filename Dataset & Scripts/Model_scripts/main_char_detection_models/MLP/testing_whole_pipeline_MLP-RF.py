...
GESTURE_MODEL_PTH = 'D:/Pycharm Projects/Hand_pose_estimation/new_project/hand_gesture_recognition/classifier_trained_on_dataset_mp_labelled/random_forest/gesture_rf_relative_scale_norm_3d.pkl'
...
# output
OUT_FOLDER = "pipeline_eval_results_MLP-RF"
...
#gesture_model.eval()
gesture_model = model = joblib.load(GESTURE_MODEL_PTH)
    ...
    if pred_bbox is not None and os.path.exists(image_path):
        ...
        if x2 > x1 and y2 > y1:
            ...
            if mp_kp is not None and len(mp_kp) == 63:
                ...
                pred_g = gesture_model(normalized_kp)
                probs = model.predict_proba(normalized_kp)
