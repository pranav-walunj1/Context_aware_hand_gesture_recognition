...
# Load trained models
#clf = joblib.load(    'D:/Pycharm Projects/Hand_pose_estimation/new_project/hand_gesture_recognition/classifier_trained_on_dataset_mp_labelled/MLP/mlp_model_30_E_scale_norm_3d.pth')
le = joblib.load(
    'D:/Pycharm Projects/Hand_pose_estimation/new_project/hand_gesture_recognition/classifier_trained_on_dataset_mp_labelled/MLP/label_encoder_30_E_scale_norm_3d.pkl')
#scaler = joblib.load(
...
# === Parameters ===
# hand gesture model
model_hand_ges = KeypointMLP(input_dim=63, hidden_dim=128, num_classes=len(le.classes_))
#model_hand_ges.load_state_dict(torch.load('D:/Pycharm Projects/Hand_pose_estimation/new_project/hand_gesture_recognition/classifier_trained_on_dataset_mp_labelled/MLP/mlp_model_30_E_scale_norm_3d.pth'))
model_hand_ges.load_state_dict(torch.load('D:/Pycharm Projects/Hand_pose_estimation/new_project/hand_gesture_recognition/classifier_trained_on_dataset_mp_labelled/MLP/mlp_model_30_E_scale_norm_3d_more_unknown_poses.pth'))
model_hand_ges.eval()
            ...
            if elapsed_time >= activation_duration:
                ...
                if rgb_crop is not None:
                    ...
                    if results.multi_hand_landmarks:
                        ...
                        sample_tensor_hand_ges = torch.tensor(normalized_keypoints, dtype=torch.float32)
                        
                        try:
                            with torch.no_grad():
                                logits_hand_ges = model_hand_ges(sample_tensor_hand_ges)
                                probs = torch.softmax(logits_hand_ges, dim=1)  # convert to probabilities
                                #pred_idx_hand_ges = torch.argmax(logits_hand_ges, dim=1).item()
                                pred_idx_hand_ges = torch.argmax(probs, dim=1).item()
                                confidence = probs[0, pred_idx_hand_ges].item()
                                gesture = le.inverse_transform([pred_idx_hand_ges])[0]
                                
                                print(f'Gesture is : {gesture} and confidence is : {confidence}')
